    ...
    with open(file_path, 'r', encoding='utf-8') as f:
        for line in f:
            ...
            for _ in range(count):
                ...
                ingredient_parts = ingr.split('|')
                ingrs['ingredient_name'] = ingredient_parts[0].strip()
                ingrs['quantity'] = int(ingredient_parts[1].strip())
                ingrs['measure'] = ingredient_parts[2].strip()
...
def rewrite_file(path1=None, path2=None, path3=None):
    if path1 or path2 or path3 is None:
        ...
        path2 = '2.txt'
        path3 = '3.txt'
        os.makedirs('sorted', exist_ok=True)  # Создание директории "sorted", если она не существует
        os.chdir('sorted')
    ...
    return print("Файл успешно создан:\n")
...
if __name__ == '__main__':
    ...
    # Откомментируйте сроку ниже для проверки работоспособности кода Задачи №1:
    #pprint(read_cookbook())

    # Откомментируйте сроку ниже для проверки работоспособности кода Задачи №2:
    #pprint(get_shop_list_by_dishes(['Запеченный картофель', 'Омлет'], 2))

    # Откомментируйте три сроки ниже для проверки работоспособности кода Задачи №3:
    #rewrite_file()
    #with open("rewrite_file.txt", 'r', encoding='utf-8') as f_total:
    #    print(f_total.read())